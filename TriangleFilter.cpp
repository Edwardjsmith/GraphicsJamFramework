...
void TriangleFilter::Dispatch(void* OutData, int dispatchCount)
{
	...
	glBufferData(GL_SHADER_STORAGE_BUFFER, m_ShaderParams.VertexDataSize, (GLvoid*)m_ShaderParams.VertexData, GL_DYNAMIC_DRAW);

	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, m_IndexBuffer);
	glBufferData(GL_SHADER_STORAGE_BUFFER, m_ShaderParams.IndexDataSize, (GLvoid*)m_ShaderParams.IndexData, GL_DYNAMIC_DRAW);
